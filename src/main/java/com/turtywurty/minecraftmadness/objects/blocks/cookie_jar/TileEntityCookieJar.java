...
public class TileEntityCookieJar extends TileEntityLockableLoot implements ITickable
{
	private NonNullList<ItemStack> jarContents = NonNullList.<ItemStack>withSize(8, ItemStack.EMPTY);
	...
	{
		for(ItemStack stack : this.jarContents)
		{
			if(!stack.isEmpty())
			{
				return false;
	...
	{
		return this.hasCustomName() ? this.customName : "container.cookie_jar";
	}
	...
	public void readFromNBT(NBTTagCompound compound) 
	{
		...
		this.jarContents = NonNullList.<ItemStack>withSize(getSizeInventory(), ItemStack.EMPTY);
	
		if(!this.checkLootAndRead(compound))
		{
			ItemStackHelper.loadAllItems(compound, jarContents);
		}
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) 
	{
		...
		{
			ItemStackHelper.saveAllItems(compound, jarContents);
		}
	...
	{
		return new ContainerCookieJar(playerInventory, this, playerIn);
	}
	
	@Override
	public String getGuiID() 
	{
		return Reference.MOD_ID + ":cookie_jar";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerCookieJar)
                {
                    if (((ContainerCookieJar)entityplayer.openContainer).getChestInventory() == this)
                    {
