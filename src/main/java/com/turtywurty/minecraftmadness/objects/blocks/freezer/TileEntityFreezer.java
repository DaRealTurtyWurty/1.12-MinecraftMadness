...
public class TileEntityFreezer extends TileEntityLockableLoot implements ITickable
{
	...
	{
		return Reference.MOD_ID + ":freezer";
	}
	...
	public void update()
	{
		if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + pos.getX() + pos.getY() + pos.getZ()) % 200 == 0)
        {
            ...
            {
                if (entityplayer.openContainer instanceof ContainerFreezer)
                {
                    if (((ContainerFreezer)entityplayer.openContainer).getChestInventory() == this)
                    {
