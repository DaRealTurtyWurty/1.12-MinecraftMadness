...
public class MadnessGenTemplate implements IChunkGenerator 
{
	protected static final IBlockState MADNESS_STONE = BlockInit.MUTATED_STONE.getDefaultState();
	protected static final IBlockState GRAVEL = BlockInit.MUTATED_STONE.getDefaultState();
	protected static final IBlockState WATER = BlockInit.MUTATED_WATER_FLUID.getDefaultState();
	...
	public MadnessGenTemplate(World worldIn, long seed) 
	{
						...
						for (int k2 = 0; k2 < 4; ++k2) {
							...
							for (int l2 = 0; l2 < 4; ++l2) {
								if ((lvt_45_1_ += d16) > 0.0D) {
									primer.setBlockState(i * 4 + k2, i2 * 8 + j2, l * 4 + l2, MADNESS_STONE);
	...
	private void generateBiomeTerrain(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal, Biome Biome) 
	{
		...
		for (int j1 = 255; j1 >= 0; --j1) 
		{
			...
			else 
			{
				...
				else if (iblockstate2.getBlock() == MADNESS_STONE.getBlock()) 
				{
					if (j == -1) 
					{
						if (k <= 0) 
			...
			for (int l = 0; l < 5; ++l) 
			{
				...
				for (int j1 = -i1; j1 <= i1; ++j1) 
				{
					for (int k1 = -i1; k1 <= i1; ++k1) 
					{
						...
						if (this.terrainType == WorldType.DEFAULT && f5 > 0.0F) 
						{
							f5 = 1.0F + f5 * 2.0F;
							f6 = 1.0F + f6 * 4.0F;
						}
